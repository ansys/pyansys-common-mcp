...
class TestPyAnsysBaseMCPDocumentation:
    """Tests verifying documentation examples."""
    ...
    def test_create_context_docstring_example(self):
        ...
        with patch('ansys.common.mcp.server.FastMCP.__init__', return_value=None):
            ...
            assert context.mapdl == "mapdl_instance"
            assert isinstance(context, PyAnsysBaseAppContext)
...
class TestPyAnsysBaseMCPErrors:
    ...
    def test_mcp_with_none_context(self):
        """Test MCP methods with None context."""
        with patch('ansys.common.mcp.server.FastMCP.__init__', return_value=None):
            mcp = MockMCP()
            mcp.context = None
            
            # Should handle gracefully
            # These methods should not crash with None context
            # (though they might raise specific errors)
